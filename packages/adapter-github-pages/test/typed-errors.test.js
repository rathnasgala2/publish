 ...
 * `PagesAdapterError` carrying a stable `.code`, never a bare `Error` a
...
import { PagesAdapterError } from '../src/errors.js';
...
test('no source file throws a bare Error; every throw is a PagesAdapterError', () => {
  const offenders = [];
  ...
  assert.deepEqual(
    offenders,
    [],
    `every documented failure in src/ must throw PagesAdapterError, not a bare Error:\n${offenders.join('\n')}`,
  );
});

test('PagesAdapterError carries a stable .code alongside .message', () => {
  const error = new PagesAdapterError('PAGES_EXAMPLE_CODE', 'detail text');
  assert.equal(error.code, 'PAGES_EXAMPLE_CODE');
  assert.equal(error.message, 'PAGES_EXAMPLE_CODE: detail text');
